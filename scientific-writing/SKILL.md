...
- **Prefer to split comma-coordinator chains, but allow one when splitting fragments.** The default is to avoid "..., but its ...", "..., so ...", "..., and X becomes ..." by ending the sentence and starting a new one, carrying cause with "therefore" between sentences. Splitting is not free. When two facts are tightly linked and splitting them produces two short sentences the reader has to reconnect, a single ", and" / ", so" is the better choice. Use it deliberately, not by habit, and never chain three clauses.
...
2. Before showing prose, self-check against the list: colons/semicolons (never), stray comma-coordinators or nested clauses (none beyond the occasional one earning its place by flow), bare demonstratives, "rather than" count, persuasive framing, repeated buzzwords, and the reader-stop test (does each short sentence connect to the next, or does the reader stall at the period?).
