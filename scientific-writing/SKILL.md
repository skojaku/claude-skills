...
- **Sparse "rather than".** At most once per passage, and watch the count across the whole section, not just within one paragraph. It recurs easily and reads as a tic. Vary contrasts with "instead", "not", "more than", or a separate sentence ("X is built for generation, not similarity search"; "produces a description, instead of the source text").
...
2. Before showing prose, self-check against the list: colons/semicolons (never), causal "..., so ..." (never), stray comma-coordinators or nested clauses (none beyond the occasional one earning its place by flow), bare demonstratives, "rather than" count (section-wide), possessive-apostrophe density, rhetorical filler, persuasive framing, repeated buzzwords, one sentence per source line, and the reader-stop test (does each short sentence connect to the next, or does the reader stall at the period?).
